...
function QuoteGenerator() {
  ...
  let quate = quotes[randomNumber];
  return quate;
}
function condition() {
  let values = QuoteGenerator();
  document.querySelector(".write").innerHTML = `  " ${values.quote}"`;
  document.querySelector(".writer").innerHTML = `Writer: ${values.source}`;
  if (values.citation) {
    document.querySelector(
      ".citation"
    ).innerHTML = `Citation: ${values.citation}`;
  }

  if (values.year) {
    document.querySelector(".year").innerHTML = `Year ${values.year}`;
  } else {
